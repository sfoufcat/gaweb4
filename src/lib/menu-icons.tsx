// Icon SVG paths for predefined menu icons
...
// Helper component to render a menu icon (either SVG or emoji)
interface MenuIconProps {
  iconKey: string;
  className?: string;
}

export function MenuIcon({ iconKey, className = "w-5 h-5" }: MenuIconProps) {
  ...
  const path = MENU_ICON_PATHS[iconKey];
  if (!path) {
    // Fallback to home icon if unknown
    return (
      <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d={MENU_ICON_PATHS.home} />
      </svg>
    );
  }
  ...
  if (iconKey === 'settings') {
    return (
      <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d={MENU_ICON_PATHS.settings} />
        <path strokeLinecap="round" strokeLinejoin="round" d={MENU_ICON_PATHS.settingsCircle} />
      </svg>
    );
  }
