...
export default function WentWellPage() {
  ...
  useEffect(() => {
    if (checkIn?.whatWentWell) {
      setText(checkIn.whatWentWell);
    }
  }, [checkIn]);
  ...
  const handleNext = async () => {
    ...
    try {
      await saveReflection('whatWentWell', text.trim());
      router.push('/checkin/weekly/obstacles');
    } catch (error) {
