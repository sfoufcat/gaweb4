...
const VALID_TRACKS: UserTrack[] = ['content_creator', 'saas', 'coach_consultant', 'ecom', 'agency', 'general'];
...
const CHECKLIST_ITEMS = [
  ...
  { 
    text: 'Choose the goal you want to pursue',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  { 
    text: 'Select the support you need',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  { 
    text: 'Receive your personalized roadmap',
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const VALUE_PROPS = [
  {
    ...
    text: 'A clarity roadmap aligned with your ambitions',
  },
  {
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    text: "Insight into what's been holding you back and how to move past it",
  },
  {
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    text: 'An invitation into a fellowship of ambitious, growth-driven minds',
  },
  {
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    text: 'A clear starting point for your next chapter',
  },
];

/**
 * Old Guest Welcome Page (preserved for reference)
 * Entry point for the legacy guest onboarding flow (/start/old)
 * No authentication required
 */
export default function OldStartPage() {
  ...
  // Set current step on mount and handle track preselection from URL
  useEffect(() => {
    if (!isLoading) {
      // Check for track query param (?track=content_creator, etc.)
      const trackParam = searchParams.get('track');
      const updateData: { currentStep: string; preselectedTrack?: UserTrack } = { currentStep: 'start' };
      
      if (trackParam && VALID_TRACKS.includes(trackParam as UserTrack)) {
        // Store preselected track in guest session
        updateData.preselectedTrack = trackParam as UserTrack;
        console.log('[START] Track preselected from URL:', trackParam);
      }
    ...
    <div className="min-h-screen bg-app-bg overflow-y-auto">
      ...
      <div className="relative px-4 py-8 lg:py-12">
        ...
        <div className="w-full max-w-xl lg:max-w-2xl mx-auto">
          ...
          >
            <h1 className="font-albert text-[24px] sm:text-[32px] lg:text-[38px] text-text-primary tracking-[-1.5px] leading-[1.1] mb-4">
              ...
              <span className="bg-gradient-to-r from-[#a07855] via-[#c9a07a] to-[#a07855] bg-clip-text text-transparent">
                Get structure, accountability, and a community of growth-oriented individuals
              </span>
            </h1>
            ...
            >
              <p className="mb-3">
                Join a micro-group of <span className="font-semibold text-text-primary">multidisciplinary thinkers and doers</span> developing themselves and building their business.
              </p>
              <p>
                And power your day with a daily system.
              </p>
            </motion.div>
            <motion.div
            ...
            >
              <div className="bg-white/70 backdrop-blur-sm border border-[#e1ddd8] rounded-2xl p-5 text-left shadow-sm">
                {/* Progress visualization - Transformation style */}
                  ...
                  >
                    ...
                    <defs>
                      ...
                      <linearGradient id="areaGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stopColor="#a07855" stopOpacity="0.15" />
                        <stop offset="100%" stopColor="#f7c948" stopOpacity="0.15" />
                      </linearGradient>
                    ...
                    <motion.path 
                      d="M 20 75 C 60 72, 100 55, 150 40 C 200 25, 240 18, 280 15 L 280 80 L 20 80 Z" 
                    ...
                    {/* Main progress curve */}
                    <motion.path 
                      d="M 20 75 C 60 72, 100 55, 150 40 C 200 25, 240 18, 280 15" 
                    ...
                    >
                      <circle cx="20" cy="75" r="6" fill="#a07855" opacity="0.2" />
                      <circle cx="20" cy="75" r="4" fill="#a07855" />
                      <circle cx="20" cy="75" r="1.5" fill="white" opacity="0.6" />
                    </motion.g>
                    ...
                    >
                      <circle cx="280" cy="15" r="10" fill="#f7c948" opacity="0.15">
                        <animate
                          attributeName="r"
                          values="10;14;10"
                          dur="2s"
                      ...
                      <circle cx="280" cy="15" r="6" fill="#f7c948" />
                      <circle cx="280" cy="15" r="2" fill="white" opacity="0.7" />
                    </motion.g>
                  </svg>

                  {/* "Today" label - positioned at start point */}
                  <motion.div 
                    className="absolute left-0 top-[calc(75%-52px)] sm:top-[calc(75%-40px)]"
                    initial={{ opacity: 0, y: 8 }}
                    animate={{ opacity: animateGraph ? 1 : 0, y: animateGraph ? 0 : 8 }}
                    transition={{ duration: 0.4, delay: 0.2 }}
                  >
                    <div className="bg-white border border-[#e1ddd8] rounded-lg px-2.5 py-1 shadow-sm">
                      <p className="font-sans text-[9px] text-text-muted uppercase tracking-wide">Today</p>
                      <p className="font-sans text-[11px] font-semibold text-text-primary">Starting Point</p>
                    </div>
                  </motion.div>
                  
                  {/* Goal label - positioned at end point */}
                  <motion.div 
                    className="absolute right-0 sm:right-2 top-[calc(15%+24px)] sm:top-[calc(15%-8px)]"
                    initial={{ opacity: 0, y: 8 }}
                    animate={{ opacity: animateGraph ? 1 : 0, y: animateGraph ? 0 : 8 }}
                    transition={{ duration: 0.4, delay: 1.3 }}
                  >
                    <div className="bg-gradient-to-r from-[#fef9e7] to-[#fef3cd] border border-[#f7c948] rounded-lg px-2.5 py-1 shadow-sm">
                      <p className="font-sans text-[9px] text-[#8b6914] uppercase tracking-wide">3 Months</p>
                      <p className="font-sans text-[11px] font-semibold text-[#5c4a0e]">Your Goal</p>
                    </div>
                  </motion.div>
                </div>
                ...
                <ul className="space-y-2">
                  <li className="flex items-start gap-3">
                    ...
                    <span className="font-sans text-[14px] lg:text-[15px] text-text-primary">A personalized plan based on your goals and ambitions</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="w-5 h-5 rounded-full bg-[#a07855]/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-[#a07855] text-[10px]">✓</span>
                    </span>
                    <span className="font-sans text-[14px] lg:text-[15px] text-text-primary">Access to a space for people who refuse to be average</span>
                  </li>
                </ul>
              </div>
            </motion.div>
          ...
          >
            Trusted by entrepreneurs, creators, and students in 40+ countries
          </motion.p>
