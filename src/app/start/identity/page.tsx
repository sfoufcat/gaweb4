...
/**
 * Post-Payment Identity Page
 * Collects the user's identity statement after account creation
 * This is for authenticated users only (after payment flow)
 */
export default function PostPaymentIdentityPage() {
  ...
  const handleSaveIdentity = async (identityToSave: string) => {
    ...
    try {
      // Save to Firebase via authenticated API
  ...
  if (!isLoaded || isSaving) {
    return (
      <div className="fixed inset-0 bg-app-bg flex flex-col items-center justify-center">
        ...
        <p className="text-text-secondary font-sans text-[15px] text-center">{isSaving ? 'Saving your identity...' : 'Loading...'}</p>
      </div>
    );
  }
        ...
        >
          <div className="w-full max-w-xl lg:max-w-2xl mx-auto">
            {canProceed ? (
              ...
              >
                {isSaving ? 'Saving...' : 'Proceed with this identity →'}
              </button>
            ) : (
              <button
