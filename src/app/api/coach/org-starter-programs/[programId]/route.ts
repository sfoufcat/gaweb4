/**
 * Coach API: Single Starter Program Management (org-scoped)
 * 
 * GET /api/coach/org-starter-programs/[programId] - Get program details with days
 * PUT /api/coach/org-starter-programs/[programId] - Update program
 * DELETE /api/coach/org-starter-programs/[programId] - Delete program
 */
...
import type { StarterProgram, StarterProgramDay, UserTrack, ProgramHabitTemplate } from '@/types';
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch starter program' }, { status: 500 });
  }
}
...
export async function PUT(
  ...
  try {
    ...
    // If setting isDefaultForTrack to true, unset other defaults for this track in this org
    ...
    return NextResponse.json({ error: 'Failed to update starter program' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (!daysSnapshot.empty) {
      ...
      await batch.commit();
      console.log(`[COACH_ORG_PROGRAM_DELETE] Deleted ${daysSnapshot.size} program days`);
    }

    // Delete the program
    await adminDb.collection('starter_programs').doc(programId).delete();

    console.log(`[COACH_ORG_PROGRAM_DELETE] Deleted program: ${programId} (${programSlug}) in org ${organizationId}`);
    ...
    return NextResponse.json({ error: 'Failed to delete starter program' }, { status: 500 });
  }
}
