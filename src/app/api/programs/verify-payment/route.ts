...
async function createCoachingRelationship(
  ...
  try {
    ...
    console.log(`[VERIFY_PAYMENT] Updated Clerk metadata for user ${userId} with coaching status`);
  } catch (metadataError) {
    console.error(`[VERIFY_PAYMENT] Failed to update Clerk metadata:`, metadataError);
  ...
  console.log(`[VERIFY_PAYMENT] Created coaching relationship: user ${userId} with coach ${coachId}`);
}
