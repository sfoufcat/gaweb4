...
import type { IntakeCallConfig, CoachAvailability, UnifiedEvent, IntakeBookingToken, OrgBranding, OrgSettings } from '@/types';
...
/**
 * POST /api/public/intake/[orgSlug]/[slug]/book
 * Book an intake call
 *
 * Body:
...
export async function POST(req: Request, { params }: RouteParams) {
  try {
    ...
    if (config.meetingProvider === 'in_app') {
      ...
      if (apiKey && apiSecret) {
        try {
          ...
          console.log(`[PUBLIC_INTAKE_BOOK] Created Stream Video call: ${streamVideoCallId}`);
        } catch (streamErr) {
          console.error(`[PUBLIC_INTAKE_BOOK] Failed to create Stream call:`, streamErr);
    ...
    sendBrandedIntakeConfirmation({
      ...
      console.error('[PUBLIC_INTAKE_BOOK] Failed to send prospect notification:', err);
    });
    ...
    sendIntakeNotificationToCoach({
      ...
      console.error('[PUBLIC_INTAKE_BOOK] Failed to send coach notification:', err);
    });
    ...
    createIntakeNotificationForCoach({
      ...
      console.error('[PUBLIC_INTAKE_BOOK] Failed to create in-app notification:', err);
    });
    ...
    console.error('[PUBLIC_INTAKE_BOOK_POST]', error);
