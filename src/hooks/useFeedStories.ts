...
import { isInReflectionWindow } from './useUserStoryAvailability';
...
interface CurrentUserStoriesResponse {
  hasStory: boolean;
  stories?: Array<{ id: string }>; // User-posted stories
  autoGeneratedData: {
    hasDayClosed?: boolean;
    hasWeekClosed?: boolean;
...
export function useCurrentUserHasStory(): CurrentUserStoryStatus {
  ...
  const status = useMemo(() => {
    ...
    const userStoryCount = data.stories?.length || 0;
    
    // Apply reflection window logic to match Home tab and other places
    const hasWeekClosed = isInReflectionWindow() && (autoData.hasWeekClosed === true);
  ...
  // Generate content hash for view tracking (must match useUserStoryAvailability hash)
  const contentHash = useMemo(
    () => generateStoryContentHash(
      status.hasTasks,
      status.hasDayClosed,
      status.taskCount,
    ...
    [status.hasTasks, status.hasDayClosed, status.taskCount, status.hasWeekClosed, status.userStoryCount]
  );
