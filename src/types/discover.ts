...
export type PurchasableContentType = 'event' | 'article' | 'course' | 'download' | 'link' | 'video';
...
export type DiscoverEvent = {
  ...
  meetingProvider?: 'zoom' | 'google_meet' | 'stream' | 'manual'; // which provider was used for the meeting
  externalMeetingId?: string; // Zoom meeting ID or Google Calendar event ID for fetching recordings
  // Recording fields
  recordingStatus?: 'recording' | 'processing' | 'ready' | 'failed' | 'encoding';
  ...
  status?: 'active' | 'canceled' | string; // event status
...
export type VideoStatus = 'uploading' | 'encoding' | 'ready' | 'failed';
...
export type DiscoverVideo = {
  ...
  // Bunny Stream fields
  bunnyVideoId: string;
  playbackUrl?: string;        // HLS URL (set by webhook after encoding)
  thumbnailUrl?: string;       // Auto-generated by Bunny
  customThumbnailUrl?: string; // Coach-uploaded thumbnail (overrides auto)
