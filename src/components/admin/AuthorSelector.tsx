      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="w-full h-10 font-normal text-left text-sm"
          ...
          >
            {selectedCoach ? (
              <div className="flex items-center gap-2 flex-1 justify-start">
                ...
                <span className="text-sm font-albert text-[#1a1a1a] dark:text-[#f5f5f8]">{selectedCoach.name}</span>
                {selectedCoach.id === currentUser?.id && (
                  <span className="text-xs font-albert text-text-secondary dark:text-[#7d8190]">(you)</span>
                )}
              </div>
            ) : (
              <span className="font-albert text-text-secondary dark:text-[#7d8190]">
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
          <div className="p-2 border-b border-[#e1ddd8] dark:border-[#262b35]">
            <input
              ...
              className="w-full px-3 py-2 text-sm font-albert border border-[#e1ddd8] dark:border-[#262b35] dark:bg-[#11141b] rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-accent dark:ring-brand-accent dark:focus:ring-brand-accent text-[#1a1a1a] dark:text-[#f5f5f8]"
            />
          </div>
          <div className="max-h-[250px] overflow-y-auto p-1">
            {loading ? (
              <div className="p-4 text-center text-sm font-albert text-text-secondary dark:text-[#7d8190]">
                Loading...
              </div>
              ...
              filteredCoaches.map(coach => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 text-sm font-albert rounded-md hover:bg-[#f3f1ef] dark:hover:bg-[#262b35] cursor-pointer text-left ${
                ...
                >
                  ...
                  <span className="text-[#1a1a1a] dark:text-[#f5f5f8] truncate flex-1">{coach.name}</span>
