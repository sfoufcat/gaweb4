...
export function CoachProgramsTab({ apiBasePath = '/api/coach/org-programs', initialProgramId, onProgramSelect }: CoachProgramsTabProps) {
  ...
  const [expandedWeeks, setExpandedWeeks] = useState<Set<number>>(new Set([0])); // Onboarding (Week 0) expanded by default
  ...
  const instanceWeeks = useMemo(() => {
    ...
    return instance.weeks.map((week, idx) => {
      ...
      return {
        ...
        // Include all editable fields that were missing
        ...
        // Use calculated calendar dates (most accurate), then stored, then undefined
  ...
  const [isCohortAdvancedOpen, setIsCohortAdvancedOpen] = useState(false);
  ...
  const [programFormData, setProgramFormData] = useState<{
    ...
    maxCallsPerWeek: 0,
    pricePerExtraCallCents: 0,
    callBookingMode: 'propose',
          ...
          try {
            ...
            if (modules.length > 0 && weeks.length === 0) {
              ...
              for (let weekIdx = 0; weekIdx < numWeeks; weekIdx++) {
                // First week (idx 0) = Onboarding (weekNum 0), then Week 1, 2, etc.
                const weekNum = weekIdx;
  ...
  const fetchCohortWeekContent = useCallback(async (programId: string, cohortId: string, weekNumber: number) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBasePath}/${programId}/cohorts/${cohortId}/week-content/${weekNumber}`);
      if (response.ok) {
        const data = await response.json();
    ...
    try {
      if (selectedProgram.type === 'individual' && clientViewContext.mode === 'client') {
        ...
        if (sidebarSelection?.type === 'week') {
          ...
          if (templateWeek) {
            await fetchCohortWeekContent(selectedProgram.id, cohortViewContext.cohortId, templateWeek.weekNumber);
          }
        }
      } else {
  ...
  useEffect(() => {
    if (cohortViewContext.mode !== 'cohort' && selectedProgram?.type === 'group') {
      setLoadedCohortId(null);
    }
  }, [cohortViewContext, selectedProgram]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      const templateWeek = programWeeks.find(w => w.weekNumber === sidebarSelection.weekNumber);
      console.log('[COHORT_CONTENT_EFFECT] templateWeek:', templateWeek?.weekNumber, 'for weekNumber:', sidebarSelection.weekNumber);
      if (templateWeek) {
        fetchCohortWeekContent(selectedProgram.id, cohortViewContext.cohortId, templateWeek.weekNumber);
      } else {
  ...
  // In template mode: always select Onboarding (week 0) as the default
  ...
  useEffect(() => {
    ...
    if (!sidebarSelection && programWeeks.length > 0 && clientViewContext.mode === 'template') {
      // Always default to Onboarding (weekNumber 0) in template mode
      // The sidebar always calculates Onboarding as the first week
      // Use stored week data if available, otherwise create virtual selection
      const onboardingWeek = programWeeks.find(w => w.weekNumber === 0);
      const firstModuleId = programWeeks[0]?.moduleId || '';

      setSidebarSelection({
        type: 'week',
        id: onboardingWeek?.id || 'week-0',
        weekNumber: 0, // Always select Onboarding
        moduleId: onboardingWeek?.moduleId || firstModuleId,
  ...
  const handleSaveSuccess = useCallback(async () => {
    ...
    // Use instance refresh when in client/cohort mode
    if (instanceId) {
    ...
    // Template mode: refresh program details
    await fetchProgramDetails(selectedProgram.id);
  }, [selectedProgram, clientViewContext.mode, cohortViewContext.mode, instanceId, refreshInstance, fetchProgramDetails]);
  ...
  const handleOpenProgramModal = (program?: Program) => {
    ...
    if (program) {
      ...
      setProgramFormData({
        ...
        maxCallsPerWeek: program.maxCallsPerWeek ?? 0,
        pricePerExtraCallCents: program.pricePerExtraCallCents ?? 0,
  ...
  const handleOpenCohortModal = (cohort?: ProgramCohort) => {
    ...
    setSaveError(null);
    setIsCohortAdvancedOpen(false); // Reset advanced dropdown state
  ...
  const handleSaveDay = async () => {
    ...
    try {
      ...
      // Template mode: save to template day
  ...
  return (
    <>
                ...
                (() => {
                  ...
                  return (
                    <WeekEditor
                      ...
                      onInstanceRefresh={async () => { await refreshInstance(); }}
                      includeWeekends={selectedProgram?.includeWeekends !== false}
                ...
                <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white/95 dark:bg-[#171b22]/95 backdrop-blur-xl border border-white/20 dark:border-white/10 p-6 shadow-2xl shadow-black/10 dark:shadow-black/30 transition-all max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4">
                    ...
                    {programFormData.type === 'individual' && (
                      ...
                      <div className="space-y-4 p-4 bg-[#faf8f6] dark:bg-[#1a1f28] rounded-lg">
                        ...
                        <div>
                          <label className="text-xs font-medium text-[#5f5a55] dark:text-[#b2b6c2] mb-1.5 block">
                            Calls per month
                          </label>
                          <div className="flex items-center gap-3">
                            <input
                              ...
                              className="w-20 px-3 py-2 border border-[#e1ddd8] dark:border-[#262b35] rounded-lg bg-white dark:bg-[#11141b] text-[#1a1a1a] dark:text-[#f5f5f8] font-albert text-center text-sm"
                            />
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3">
                            <input
                              ...
                              min="0"
                              max="7"
                              value={programFormData.maxCallsPerWeek}
                              onChange={(e) => setProgramFormData({
                                ...programFormData,
                                maxCallsPerWeek: Math.max(0, parseInt(e.target.value) || 0)
                              })}
                              className="w-20 px-3 py-2 border border-[#e1ddd8] dark:border-[#262b35] rounded-lg bg-white dark:bg-[#11141b] text-[#1a1a1a] dark:text-[#f5f5f8] font-albert text-center text-sm"
                            />
                            <span className="text-xs text-[#a7a39e] dark:text-[#7d8190]">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-[#5f5a55] dark:text-[#b2b6c2]">$</span>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                              value={(programFormData.pricePerExtraCallCents / 100).toFixed(2)}
                              onChange={(e) => setProgramFormData({
                                ...programFormData,
                                pricePerExtraCallCents: Math.max(0, Math.round(parseFloat(e.target.value) * 100) || 0)
                              })}
                              className="w-24 px-3 py-2 border border-[#e1ddd8] dark:border-[#262b35] rounded-lg bg-white dark:bg-[#11141b] text-[#1a1a1a] dark:text-[#f5f5f8] font-albert text-center text-sm"
                            />
                            <span className="text-xs text-[#a7a39e] dark:text-[#7d8190]">
                              when allowance exceeded
                            </span>
                          </div>
                        </div>
                        ...
                        {/* Booking Mode */}
                        <div>
                          <label className="text-xs font-medium text-[#5f5a55] dark:text-[#b2b6c2] mb-2 block">
                            Booking mode
                          </label>
                          <div className="space-y-2">
                            <label className="flex items-start gap-2 cursor-pointer group">
                              <input
                                type="radio"
                                name="callBookingMode"
                                value="propose"
                                checked={programFormData.callBookingMode === 'propose'}
                                onChange={() => setProgramFormData({
                                  ...programFormData,
                                  callBookingMode: 'propose'
                                })}
                                className="mt-0.5"
                              />
                              <div>
                                <span className="text-sm text-[#1a1a1a] dark:text-[#f5f5f8] group-hover:text-brand-accent">
                                  Client proposes times
                                </span>
                                <p className="text-xs text-[#a7a39e] dark:text-[#7d8190]">
                                  You review and confirm each request
                                </p>
                              </div>
                            </label>
                            <label className="flex items-start gap-2 cursor-pointer group">
                              <input
                                ...
                                name="callBookingMode"
                                value="direct"
                                checked={programFormData.callBookingMode === 'direct'}
                                onChange={() => setProgramFormData({
                                  ...programFormData,
                                  callBookingMode: 'direct'
                                })}
                                className="mt-0.5"
                              />
                              <div>
                                ...
                                <p className="text-xs text-[#a7a39e] dark:text-[#7d8190]">
                                  Clients book directly into your availability
                                </p>
                              </div>
                            </label>
                          </div>
                        ...
                        <p className="text-xs text-[#a7a39e] dark:text-[#7d8190] border-t border-[#e1ddd8] dark:border-[#262b35] pt-3 mt-3">
                          Set calls per month to 0 for unlimited or pay-per-call pricing.
                        </p>
                      </div>
                    </div>
                    )}
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          // When dialog is dismissed (clicking outside or Escape key), cancel the operation
          if (!open) {
      ...
      >
        <AlertDialogContent>
          {/* X close button */}
          <button
            onClick={handleLeaveWarningCancel}
            className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
          >
            <X className="h-4 w-4" />
            <span className="sr-only">Close</span>
