...
interface SendDMModalProps {
  ...
  channelId?: string; // Existing channel to send to (e.g., coaching channel)
}
 ...
 * - If channelId is provided, sends to existing channel
 * - Otherwise creates/uses DM channel via bulk API
 * - Responsive: Modal on desktop, bottom sheet on mobile
 */
export function SendDMModal({ recipients, onClose, onSuccess, channelId }: SendDMModalProps) {
  ...
  // Focus textarea on mount (with delay for animation)
  useEffect(() => {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      // If we have an existing channel, send directly to it
      if (channelId && recipients.length === 1) {
        ...
        setResults([{ userId: recipients[0].userId, success: true }]);
        ...
        // Use bulk API for multiple recipients or when no channel exists
  ...
  }, [hasValidMessage, isSending, recipients, message, onSuccess, onClose, channelId]);
      ...
      <Dialog as="div" className="relative z-[60]" onClose={onClose}>
        ...
        <div className="fixed inset-0 overflow-y-auto">
          ...
          <div className="flex min-h-full items-end sm:items-center justify-center sm:p-4">
            ...
            >
              <Dialog.Panel
                className="w-full sm:max-w-md transform overflow-hidden rounded-t-3xl sm:rounded-2xl bg-white dark:bg-[#171b22] shadow-xl transition-all max-h-[90vh] sm:max-h-[85vh] flex flex-col"
              ...
              >
                ...
                {/* Header */}
                <div className="px-6 pt-2 sm:pt-6 pb-4 border-b border-[#e8e4df] dark:border-[#262b35] flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Dialog.Title className="font-semibold text-[18px] text-[#1a1a1a] dark:text-[#faf8f6] font-albert">
                          ...
                          <>
                            ...
                            <div>
                              <p className="font-semibold text-[#1a1a1a] dark:text-[#f5f5f8] font-albert">
                                {isBulk ? 'Messages Sent!' : 'Message Sent!'}
                              </p>
                              <p className="text-[13px] text-[#8a857f] font-albert">
                                {isBulk
                                  ? `Successfully sent to ${successCount} recipient${successCount !== 1 ? 's' : ''}`
                                  : `Message delivered to ${recipients[0]?.name}`
                                }
                              </p>
                            </div>
                          </>
                        ...
                        <div className="flex items-center gap-3 p-4 bg-[#faf8f6] dark:bg-[#11141b] rounded-xl">
                          ...
                          <div className="flex-1">
                            <p className="text-[13px] font-medium text-[#1a1a1a] dark:text-[#f5f5f8] font-albert">
                              {isBulk ? 'Sending messages...' : 'Sending message...'}
                            </p>
                            {isBulk && (
                              <div className="mt-2">
