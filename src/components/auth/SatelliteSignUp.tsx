...
interface SatelliteSignUpProps {
  ...
  logoUrl: string;
  appTitle: string;
  redirectUrl?: string; // Where to redirect after sign-up (e.g., /join/callback?flowSessionId=xxx)
}
 ...
 * 
 * If user is already signed in, shows "Continue as X" screen instead of sign-up form.
 */
export function SatelliteSignUp({ subdomain, customDomain, logoUrl, appTitle, redirectUrl = '/' }: SatelliteSignUpProps) {
  ...
  const iframeSrc = `${subdomainBase}/join/embedded?origin=${encodeURIComponent(currentOrigin)}&redirectUrl=${encodeURIComponent(redirectUrl)}`;
  ...
  const handleSignOut = async () => {
    ...
    try {
      await signOut();
      // After sign out, the page will re-render and show sign-up form
    } catch (error) {
            ...
            <div className="w-full max-w-lg mx-auto">
              <div className="bg-white/80 backdrop-blur-sm border border-[#e1ddd8]/60 rounded-3xl p-8 shadow-lg">
                ...
                <div className="space-y-4">
                  ...
                  >
                    {signingOut ? 'Signing out...' : 'Sign up with a different account'}
                  </button>
                </div>
              </div>
            </div>
    ...
    <div className="fixed inset-0 bg-app-bg overflow-y-auto">
      <div className="min-h-full flex flex-col items-center justify-center px-4 py-8 lg:py-16">
        <div className="w-full max-w-xl mx-auto">
          ...
          {/* Sign In Link - preserve redirectUrl for sign-in flow */}
