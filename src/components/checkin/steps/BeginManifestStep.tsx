...
import type { BeginManifestStepProps } from './types';

/**
 * BeginManifestStep - Transition screen before manifestation
 *
 * Extracted from /src/app/checkin/morning/begin-manifest/page.tsx
 * Features:
 * - "Time to manifest" messaging
 * - Visualization instructions
 * - Simple transition to the manifest step
 */
export function BeginManifestStep({ config, onComplete }: BeginManifestStepProps) {
      ...
      <div className="flex-1 flex flex-col md:items-center md:justify-center px-6 pt-8 md:pt-0 w-full overflow-y-auto">
        <div className="max-w-[500px] w-full flex-1 md:flex-initial flex flex-col text-center">
          ...
          <h1 className="font-albert text-[32px] md:text-[42px] text-[#1a1a1a] dark:text-white tracking-[-2px] leading-[1.2] mb-8">
            {config.heading || 'Time to manifest'}
          </h1>

          {/* Content */}
          <div className="space-y-6">
            <p className="font-sans text-[18px] md:text-[20px] text-[#1a1a1a] dark:text-white/90 tracking-[-0.4px] leading-[1.5]">
