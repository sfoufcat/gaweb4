...
import type { AcceptStepProps } from './types';

// Dynamic headings based on emotional state (from original accept/page.tsx)
...
/**
 * AcceptStep - Acceptance and acknowledgment step
 *
 * Extracted from /src/app/checkin/morning/accept/page.tsx
 * Features:
 * - Dynamic headings based on emotional state from session data
 * - Supportive messaging
 * - Simple layout with continue button
 */
export function AcceptStep({ config, data, onComplete }: AcceptStepProps) {
  const emotionalState = (data?.emotionalState as EmotionalState) || 'uncertain';
  const heading = config.heading || HEADINGS[emotionalState] || 'Feeling uncertain today?';
        ...
        <div className="max-w-[500px] w-full flex-1 md:flex-initial flex flex-col text-center">
          ...
          <div className="space-y-6">
            <p className="font-sans text-[18px] md:text-[20px] text-[#1a1a1a] dark:text-white/90 tracking-[-0.4px] leading-[1.4]">
              {config.message || "It's okay. Doubt is part of the journey, even for the most driven."}
            </p>
