...
import type { WeeklyProgressStepProps } from './types';

/**
 * WeeklyProgressStep - Momentum-based progress slider
 *
 * 0-100% horizontal progress slider with momentum physics,
 * velocity-based animation, snap to 5% increments, and audio feedback.
 * Extracted from: src/app/checkin/weekly/evaluate/page.tsx
 */
export function WeeklyProgressStep({ config, onComplete }: WeeklyProgressStepProps) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // Pass progress and goal achieved flag
      onComplete({
        weeklyProgress: finalProgress,
        goalAchieved: finalProgress >= goalAchievedThreshold,
      });
    } catch (error) {
  ...
  const finalProgress = Math.round(displayProgress / 5) * 5;
  const isGoalComplete = finalProgress >= goalAchievedThreshold;
