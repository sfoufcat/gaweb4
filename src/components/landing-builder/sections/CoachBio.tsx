        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center">
            ...
            <div className="flex-1">
              ...
              <p className="text-lg text-brand-accent font-medium mb-4">{title}</p>
              ...
              {ctaText && (
                <a
                  href={ctaUrl}
                  className="inline-flex items-center justify-center px-6 py-3 mt-6 text-white bg-brand-accent hover:bg-brand-accent/90 rounded-lg font-medium transition-colors"
                >
                  {ctaText}
                </a>
              )}
    ...
    <section className="py-16 px-6">
      <div className="max-w-lg mx-auto">
        <div className="bg-card border border-border rounded-2xl p-8 text-center">
          ...
          <p className="text-brand-accent font-medium mb-4">{title}</p>
          ...
          {showCredentials && credentials.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {credentials.map((credential, index) => (
                <span
                  key={index}
                  className="px-2 py-0.5 bg-brand-accent/10 text-brand-accent rounded-full text-xs font-medium"
                >
                  {credential.text}
                </span>
              ))}
          ...
          {ctaText && (
            <a
              href={ctaUrl}
              className="inline-flex items-center justify-center px-6 py-3 mt-6 text-white bg-brand-accent hover:bg-brand-accent/90 rounded-lg font-medium transition-colors w-full"
            >
              {ctaText}
            </a>
          )}
