...
import { Check, Loader2, Sparkles, AlertCircle, CheckCircle2, PartyPopper } from 'lucide-react';
import type { FunnelStepConfigUpsell } from '@/types';
...
export function UpsellStep({
  ...
  useEffect(() => {
    ...
    fetch(`/api/programs/${config.productId}/price`)
      ...
      .catch(err => {
        console.error('[UPSELL_STEP] Failed to fetch product price:', err);
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      console.error('[UPSELL_STEP] Error:', err);
    ...
    <div className="w-full max-w-lg mx-auto">
      ...
      >
        ...
        >
          ...
          <span>Special One-Time Offer</span>
        </div>
      </motion.div>
      ...
      >
        ...
        >
          {config.declineText || 'No thanks, skip this offer'}
        </button>
      </motion.div>
