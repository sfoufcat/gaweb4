...
export function EventCard({ event, isPast = false }: EventCardProps) {
  ...
  // For paid content (non-past events), render a clickable div that opens the purchase sheet
  if (!isFree && !isPast) {
    return (
      <>
        <div 
          onClick={() => setShowPurchaseSheet(true)}
          className={`bg-white dark:bg-[#171b22] rounded-[20px] w-[180px] flex-shrink-0 hover:shadow-md dark:hover:shadow-black/30 transition-shadow cursor-pointer overflow-hidden ${isPast && !hasRecording ? 'opacity-70' : ''}`}
        >
          <CardContent />
        </div>
  ...
  // For free content or past events, use Link for direct navigation
  return (
