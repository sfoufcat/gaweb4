...
interface CoachfulLogoProps {
  className?: string;
  animated?: boolean;
}

/**
 * Animated Coachful Logo
 * 
 * A stylized "C" with an upward arrow integrated into the design.
 * Features a light animation that sweeps through the arrow to create
 * a dynamic, energetic effect.
 */
export function CoachfulLogo({ className = 'w-8 h-8', animated = true }: CoachfulLogoProps) {
  // Unique ID for the gradient to avoid conflicts when multiple logos are rendered
  const gradientId = `cf-light-sweep-${Math.random().toString(36).slice(2, 9)}`;
