...
export function TaskArchiveModal({ isOpen, onClose, onRestore }: TaskArchiveModalProps) {
  ...
  const closeButton = (
    <button
      onClick={onClose}
      className="w-full py-3 rounded-full bg-[#f3f1ef] dark:bg-[#1d222b] text-text-primary dark:text-[#f5f5f8] font-sans font-medium text-[14px] hover:bg-[#e8e4e0] dark:hover:bg-[#262b35] transition-colors"
    >
      Close
    </button>
  );
  ...
  if (isDesktop) {
    return (
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="max-w-[450px] max-h-[85vh] flex flex-col p-0">
          ...
          <DialogScrollArea className="px-6">
            {taskListContent}
          </DialogScrollArea>
          <DialogFooter>
            {closeButton}
  ...
  return (
    <Drawer open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DrawerContent className="max-w-[500px] mx-auto max-h-[90vh] flex flex-col">
        <DrawerHeader className="px-6 pt-5 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DrawerTitle className="font-albert text-[20px] font-medium text-text-primary dark:text-[#f5f5f8] tracking-[-1px]">
                Archived Tasks
              </DrawerTitle>
              <p className="font-sans text-[12px] text-text-muted dark:text-[#7d8190]">
                Tasks are permanently deleted 30 days after archive
        ...
        <DrawerScrollArea className="px-6">
          {taskListContent}
        </DrawerScrollArea>
        <DrawerFooter>
          {closeButton}
